...
try:
	theta = np.loadtxt((foldername + 'line_ori_'+str(ii)+'.txt'), unpack=True)
except IOError as e:
	print('line_ori_'+str(ii)+'.txt not found')
numtheta = theta.shape[1]
print(numtheta)
#print(type(theta))
#print(theta[0:,0])
...
p4, = par3.plot(x, XYZ[2,1:], 'k-', label='height of reconstructed line node',marker='v',markeredgewidth=0,markersize=6)
...
plt.savefig((foldername+'Test_ImgNum.png'), bbox_inches="tight", dpi=300)
...
plt.subplots_adjust(bottom=0.3)
plt.gcf().set_size_inches(10, 7)
plt.savefig((foldername+'Test_SigmaXXhat.png'), bbox_inches="tight", dpi=300)
...
plt.subplots_adjust(bottom=0.3)
plt.gcf().set_size_inches(10, 7)
plt.savefig((foldername+'Test_XY_Z_hat.png'), bbox_inches="tight", dpi=300)
...
#plt.tight_layout()
plt.savefig((foldername+'Test_ImgNumSigmaXXCorrelation.png'), bbox_inches="tight", dpi=300)



plt.clf()

####### plot 7: line orientation in image spaces
...
plt.legend(loc='lower left', bbox_to_anchor=(1.01,0.1),fontsize=9)

plt.xlim([1,numtheta+1])

plt.xlabel('the i$^{th}$ node')

plt.ylabel('angle')

plt.subplots_adjust(right=0.8)
plt.gcf().set_size_inches(8, 6)
plt.savefig((foldername+'Test_LineOri.png'), bbox_inches="tight", dpi=300)
